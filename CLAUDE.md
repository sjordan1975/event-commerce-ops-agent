...
**Step 8 merged to `main`. All 9 capabilities complete. Next action: `pyproject.toml` dependency audit — verify a clean install works in a fresh venv, then promote `google-genai`, `mcp`, and `anyio` from transitive to explicit deps (direct imports in `src/` and tests deserve explicit pinning). After that: back-third delivery roadmap (demo corpus, live wiring, approval UI, Cloud Run, demo video) — stub `docs/plans/` roadmap per `project_delivery_roadmap_todo` memory.**
