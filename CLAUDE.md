...
**Dir-path workflow live-verified + UI hardening (committed `ee771a0`).** Validated the `list_images` → `run_event_pipeline` coordinator path with real local files in `/tmp/wc-final/`. Three bugs found and fixed: (1) thumbnails broken for local-path `content_url` — added `GET /api/image?path=` proxy endpoint in `server.py` and `rewritePhotoUrl()` in `live-api.ts` (applied in `approval_ready` and `execution_evidence` SSE handlers); (2) t-shirt mockup spinner never resolved for local files — `_fetch_image_bytes` in `execution.py` now reads local paths via `Path.read_bytes()` before falling through to `urlopen`; (3) no UX for transient LLM errors (Gemini 503/429 cause silent indefinite hang at any LLM step — `propose_review_queue`, `score_assets_with_vision`, `_draft_copy_for_asset`, coordinator itself) — added `phase='error'` to `PipelinePhase`, SSE `error` events now throw in `live-api.ts`, `usePipeline` catch sets error state, `ContentPane` shows error panel with "Try again" (auto-resubmits last prompt) + "or type a new prompt below" hint; `ChatBar` enabled in error state. Retry does not require `reset_atlas.py` — partial MongoDB data from failed run is benign (orphaned under a new `event_id`). **Operational:** `_drain_queue` timeout is 300s; Gemini SDK retries silently before surfacing, so visible hang can be several minutes before error panel appears.

**Immediate next:** live Shopify/Printful wiring — swap the execution stubs in Step 7 with real API calls. Then: Cloud Run deploy, demo video. Demo reset between runs = `reset_atlas.py` (sufficient; no separate prep script needed). Pre-demo close tasks in `docs/project-close-hygiene.md`. Deadline: June 11, 2026 @ 2:00 PM PDT.
