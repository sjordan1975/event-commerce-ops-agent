...
**Seed data curated (committed `3c3e038`).** `SEED_IMAGES` extracted to `scripts/seed_images.py` with human-reviewed `product_route` on all 40 entries (no nulls: 3 tshirt / 6 poster / 31 social_only). `--patch-routes` mode applied to Atlas: 24 assets corrected. Routing mechanics are now documented precisely in `docs/specs/01-requirements.md` § "Routing mechanics (precise)" — do not re-derive in conversation; read that section. Key points: exploitation routes are code-locked (similarity-weighted plurality vote, LLM cannot override); discovery routes are LLM judgment from live Step 4 vision scores (seed corpus irrelevant to discovery); discovery routing is uncalibrated — a proper eval requires a human-scored reference set (enterprise-path concern).

**Immediate next:** enrich commercial assets in seed corpus — 3 steps: (1) add tshirt/poster images to `scripts/seed_images.py` (must use an existing seed `event_id`); (2) add `--add-images` mode to seeder (inserts asset + embedding + performance for URLs not yet in Atlas — idempotency check blocks default re-run); (3) update `reset_atlas.py` hardcoded expected counts. Then re-run browser test to verify commerce routing → test server-side mockup generation (t-shirt + poster paths in `execution.py`) → demo corpus prep (`scripts/prep_demo_corpus.py`), live Shopify/Printful wiring, Cloud Run deploy, demo video. Pre-demo close tasks in `docs/project-close-hygiene.md`. Deadline: June 11, 2026 @ 2:00 PM PDT.
