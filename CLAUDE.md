...
**On branch `step/5-queue` (cut from `main`). Step 5 plan + tasks + branch housekeeping COMMITTED; implementation NOT started. Next action: implement T-5.1 (the `QueueItem`/`ReviewQueue` models).**

Step 5 = `propose_review_queue`, the one strategic decision (D-021). Three commits on the branch: plan (`ffdf266`), housekeeping/D-029 (`f49e0be`), tasks (`1959ece`). Next: work `docs/tasks/step-5-tasks.md` in order, T-5.1 → T-5.14 (T-5.0 housekeeping already done).

Key Step 5 decisions baked in (D-029): `propose_review_queue` is the project's **first in-graph `LlmAgent(mode='single_turn')` node** (Path A — validated by `spike/adk_llm_node_queue_spike.py`, local/gitignored, 20/20 live pass-rate probe). Realized as **three nodes**: `prepare_queue_candidates` (FunctionNode — mechanical similarity-cutoff split + all precondition checks) → `propose_review_queue` (LlmAgent — judgment) → `persist_review_queue` (FunctionNode — defensive Mongo write). New env vars: `GEMINI_QUEUE_MODEL` (default `gemini-2.5-flash`), `QUEUE_EXPLOITATION_SIMILARITY_CUTOFF` (default `0.75`, corpus-tuned). Exploitation routing stays mechanical (`inferred_route`, D-015); discovery routing is the LLM's choice. Persists `queue_type` (enum value `"discovery"`, not "exploration"), `product_route`, `queue_rank`, `queue_rationale` onto assets; **no status change** (stays `"scored"`). Node mechanics: instruction = callable provider reading `ctx.state` (ADK uses it verbatim — JSON-safe); `output_schema=ReviewQueue` + `output_key` lands a parsed dict; eval mock seam = `before_model_callback` returning a canned `LlmResponse` (`_FIXTURE_RESPONSE`).

**Two-tier eval (D-029 + user steer):** Tier 1 (`tests/evals/test_step_5_trace.py`) is deterministic/mocked, zero live calls — the **offline CI ship gate**, isolating code correctness from AI-infra flakiness (503/rate-limit). Tier 2 (`tests/evals/test_step_5_coherence.py`) runs the strategic node **live** — the D-020 95%/20-run judgment gate, run deliberately (not on CI); transient API errors retried/excluded. Eval construction = **full `run_event_pipeline` dispatch with upstream LLM helpers patched** (so only the strategic node is live in Tier 2; one live call/run).

Reference: `docs/plans/step-5-queue.md` (plan), `docs/tasks/step-5-tasks.md` (T-5.0–T-5.14), `tracking.md` D-029, `docs/strategic-agent-reframe.md` for the capability surface + queue-assembly design.
