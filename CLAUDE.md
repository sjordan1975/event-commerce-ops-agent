...
**All 9 capabilities complete; in the back-third (demo prep). This session hardened the MongoDB-MCP integration — which had never been exercised against the real server (everything was mock-validated until now). Landed on `main`: D-034 (vector-index name + in-query `event_id` exclusion filter, read-wrapper `_id` strips, vestigial `scored_at`/`published_at` prune), D-035 (persistent-MCP FastAPI backend in `src/api` — lifespan-warmed session, `/health`, `/api/asset-count`; boot ~1.9s then ~40ms/call reused), the MCP envelope-parse fix (`_parse_docs_response` was matching the security preamble's inline tag refs → the entire find/aggregate read path now works against the real server), and capture-from-reality MCP fixtures + hermetic contract tests (`scripts/capture_mcp_fixtures.py`, `tests/test_mcp_contract.py`, `tests/test_live_smoke.py`).**
...
**Immediate next (Phase A, on `ui/phase-a`): the operator console is already built — the only remaining mock-first pieces are (1) the MCP health badge (`docs/plans/approval-ui-spec.md` § MCP connection health) and (2) credential-gated preview cards (same doc, Beat 4 § Preview mode). Both are mock-first; their shape is the visual contract for the Phase-B backend. Heed `ui/AGENTS.md` — this is a customized Next.js; read its bundled docs before writing UI code. THEN Phase B: wire the Next.js `ui/` to the real `src/api` backend; then remaining tracks (live Shopify/Printful execution wiring, Cloud Run deploy, demo video). Pre-demo close tasks in `docs/project-close-hygiene.md`.**
