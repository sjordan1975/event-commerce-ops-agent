...
**`test_step_1_trace.py` — recast as the coordinator e2e smoke (FIXED + live-verified, `1 passed`).** It is the project's *one* e2e tier: the only eval that drives the real coordinator (NL → `run_event_pipeline` → the full 9-node graph → present results → advance to the HITL gate); every other capability eval dispatches the workflow directly. The prior "known failure" diagnosis (eval scaffold mishandles the coordinator tool sequence) was wrong — root cause was **seed rot**: `_seed_mock_for_full_pipeline` covered a 2-node pipeline while the graph grew to 9, so downstream nodes hit unseeded reads and aborted. Fixes: (1) seed every node's reads (reuses Step-6 scaffolding + empty canned ReviewQueue + `_stub_internal_llms` for narrative/copy LLMs + `approvals.get_client` patch so `request_human_approval` suspends cleanly); (2) a **completion forcing function** (`_assert_pipeline_completed`) that fails loud and names the aborting node if a future node's reads aren't seeded — replacing the silent-rot-then-normalize pattern; (3) assertion (f) relaxed from `is_final` text to "non-empty text after `run_event_pipeline` returns" because the smoke's natural terminus is an HITL suspension, not a final-text turn. The smoke's only live calls are the coordinator's 2 (dispatch + terminal); like all live evals it is subject to transient Gemini 503s (google-genai retries with backoff, so a throttled run *silently hangs for minutes* with 0-byte output — do NOT launch live evals concurrently, it causes a 503 storm). Unit tests (deterministic, no live calls): `pytest tests/ --ignore=tests/evals/`.
...
**Immediate next (Phase B, on `ui/phase-b`):** SSE pipeline wire-up (`ui/src/lib/live-api.ts`) — the same env-var gate that switches health polling also switches the pipeline from fixture simulation to the real SSE stream. The SSE event→callback mapping is specced in `docs/plans/approval-ui-spec.md` § Real-time data contract. Start backend with `.venv/bin/python3 -m uvicorn src.api.server:app --port 8000` (reads `.env`). For the first SSE test, use explicit Wikimedia URLs (no local files needed); the coordinator can also enumerate a local dir or `gs://` path via the new `list_images` tool. THEN: demo corpus prep (`scripts/prep_demo_corpus.py`), live Shopify/Printful wiring, Cloud Run deploy, demo video. Pre-demo close tasks in `docs/project-close-hygiene.md`. Deadline: June 11, 2026 @ 2:00 PM PDT (9 days).**
