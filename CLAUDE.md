...
**Step 6 merged to `main` (`d2205c7`). Currently on `main`. Next action: cut `step/7-hitl`, write the Step 7 plan for `request_human_approval` + `execute_approved_campaigns` (capabilities 7/8).**

Step 6 = `draft_campaigns_for_queue` (capability 6) — complete. 118 unit tests + Tier 1 gate + grounding probe all green. Prompt strengthened after live grounding probe caught hallucination guard violation (first-rung fix: GROUNDING GUARD moved to top + explicit "(none)" rendering for empty detected_subjects + restored "no invented action/pose/moment" clause).
