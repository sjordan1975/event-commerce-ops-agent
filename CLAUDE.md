...
**IMMEDIATE: Seed data cleanup (blocks commerce routing + mockup testing).** Live test showed both images routed `social_only`. Root cause confirmed in Atlas: ~15 of 40 seed assets have `product_route: null` (dead weight in similarity context), and the two Messi-like seed assets (`Lionel_Messi_20180626.jpg` → null, `Lionel_Messi_celebrating...Iran_2014.jpg` → social_only) bias Step 5 away from commerce for player-portrait test images. Only one clear player-portrait-to-tshirt example in corpus (`Cristiano_Ronaldo_0876.jpg`). Fix: (1) resolve `null` product_routes in `scripts/seed_mongodb.py` to real values; (2) update Atlas directly or re-run seeder; (3) ensure player portraits carry `tshirt` route so Step 5 has usable commerce priors. Until fixed, commerce-path (t-shirt/poster) mockup generation is untestable. Separate test hygiene note: `Messi_vs_Nigeria_2018.jpg` test URL is not a seed URL verbatim but `Lionel_Messi_20180626.jpg` (2018 WC, same kit) lands close in embedding space — use clearly distinct test images post-fix.

**Immediate next:** seed data fix (above) → re-run browser test to verify commerce routing → test server-side mockup generation (t-shirt + poster paths in `execution.py`) → demo corpus prep (`scripts/prep_demo_corpus.py`), live Shopify/Printful wiring, Cloud Run deploy, demo video. Pre-demo close tasks in `docs/project-close-hygiene.md`. Deadline: June 11, 2026 @ 2:00 PM PDT.
