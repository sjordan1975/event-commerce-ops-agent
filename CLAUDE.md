...
**Two-tier eval:** Tier 1 (`test_step_5_trace.py`) dispatches the workflow directly (no coordinator) with `_FIXTURE_RESPONSE` set and all LLM surfaces patched — **truly zero live calls, no GOOGLE_API_KEY required** (this is the "offline CI ship gate" per D-029). T1-e (coordinator dispatch + CoT) is intentionally omitted — it's a coordinator assertion, already covered by Steps 2–4 trace evals. Tier 2 (`test_step_5_coherence.py`) runs the strategic node live — D-020 95%/20-run judgment gate, run deliberately with GOOGLE_API_KEY (the spike ran 20/20, but the production prompt path was not exercised by the spike — Tier 2 must be run before merge).

Reference: `docs/plans/step-5-queue.md`, `docs/tasks/step-5-tasks.md`, `tracking.md` D-029.
