...
OPERATOR_PROMPT = (
    "The event with id evt-demo-1 has been ingested. Build context for it."
)
...
def _make_events_find_handler():
    ...
    def handler(args: dict) -> list:
        ...
        if isinstance(event_id_filter, str):
            # get_event: exact match by event_id
            return [e for e in [_SEEDED_EVENT] if e["event_id"] == event_id_filter]

        if outcome_filter is not None and isinstance(event_id_filter, dict) and "$ne" in event_id_filter:
            # find_past_events_by_outcome: filter by outcome_type, exclude event_id
...
def _assert_single_run(events: list, mock_client, trace_label: str) -> list[str]:
    ...
    context_calls = [c for c in tool_calls if c["name"] == "build_event_context"]
    ingest_calls = [c for c in tool_calls if c["name"] == "ingest_event_batch"]

    # (a) build_event_context called exactly once
    if len(context_calls) != 1:
        failures.append(
            f"(a) Expected build_event_context called 1 time, got {len(context_calls)}. "
            f"Trace: {trace_path}"
        )

    # (b) ingest_event_batch NOT called
    if ingest_calls:
        failures.append(
            f"(b) ingest_event_batch should NOT be called (event already ingested), "
            f"but was called {len(ingest_calls)} time(s). Trace: {trace_path}"
        )

    # (c) event_id argument is "evt-demo-1"
    if context_calls:
        arg_event_id = context_calls[0]["args"].get("event_id")
        if arg_event_id != "evt-demo-1":
            failures.append(
                f"(c) event_id arg: expected 'evt-demo-1', got {arg_event_id!r}. "
                f"Trace: {trace_path}"
            )
