...
### D-031 — Capabilities 7/8 Coordinator-Plane + HITL Resume Mechanics

**Date:** 2026-05-29
**Decision:** Step 7 delivers capabilities **7 (`request_human_approval`)** + **8 (`execute_approved_campaigns`)** plus the **redraft loop** deferred from Step 6 (closes the D-030 `operator_notes` reservation). It is the **first step whose capabilities live coordinator-side, not in the workflow graph** — it adds **zero** graph nodes (`build_pipeline_graph()` stays at 9). The pipeline ran to `END` (Step 6 drafts); the coordinator picks up: suspends on approval, persists per-item decisions, loops redraft on `edit_requested`, then executes the approved set. Reference: `docs/plans/step-7-hitl.md`, `docs/tasks/step-7-tasks.md`.

**The resume-payload spine (corrects two source docs).** A `LongRunningFunctionTool` body returns `None`/an initial payload **once** and suspends; on resume the operator's `FunctionResponse.response` is delivered **to the coordinator LLM as the tool result** — the function body **does not re-run** (verified by `spike/adk_hitl_test.py`, `spike/adk_workflow_hitl_spike.py` claim 2). So `request_human_approval` *cannot* record decisions on resume. This **corrects** `db-wrapper-inventory.md`:210 ("`record_approval_decision` called by `request_human_approval` on resume" — false) and `02-architecture.md`:287-293 (which attributed the decision writes to the gate). Resolution: the gate is a pure read + suspend (no decision writes); a **separate `apply_approval_decisions` tool** persists decisions on the next LLM turn; `execute_approved_campaigns` and redraft are **pure consumers of persisted Mongo state** (`status="approved"` / `status="edit_requested"`) — same D-022 persistence philosophy.

**Sub-decisions:**
1. **(a) Coordinator-plane, not a workflow node.** Capabilities 7/8 are coordinator `FunctionTool`s (like `run_event_pipeline`); zero new graph nodes. HITL suspension halts the *operator conversation* plane, which is the coordinator's (`02-architecture.md`:389).
2. **(b) The spine** (above) — separate gate / apply-tool / state-consumers. The operator decisions arrive as a **per-item list** keyed by `approval_id`: `{decisions:[{approval_id, decision, reviewer_notes}]}`, buckets `approved | rejected | edit_requested`. (The list payload — vs. the spikes' flat single dict — is the one new ADK fact, verified by Tier-1 assertion, not a new spike.)
3. **(c) Resolve-then-execute** (canonical mixed-batch flow): loop `redraft → request_human_approval` until no `edit_requested` remain (or the cap), **then one** `execute_approved_campaigns`. Approvals **accumulate** across cycles (`approved` items wait; the final execute reads them all from Mongo). Interleaved is the documented alternative (user veto point).
4. **(d) Redraft is a state-consumer** — reads persisted `edit_requested` approvals (notes included); `operator_notes` retained **only** for signature stability + unit injection, off the production path (reconciles D-030's deferral).
5. **(e) Gap 1 closed** (`safety-measures.md`) — tool-level redraft cap in `tool_context.state` (`MAX_REDRAFT_CYCLES`, default 3, robust to LLM mis-ordering) + prompt 3-cycle rule (ported from the v2 prompt to v3 coordinator) + an explicit **ADK iteration-cap value (30)** as the framework backstop (the load-bearing bound is the redraft cap = 3).
6. **(f) Operator-presence / no-timeout** (`01-requirements.md`:149) — suspends, never exits; no wall-clock timer; holds until the operator responds. "No timeout" ≠ "no bound" (the redraft loop is capped, orthogonally). Honest boundary: with `InMemorySessionService` the park is **process-lifetime-bound**; durable cross-restart resume needs a persistent session service (Cloud Run / enterprise).
7. **(g) Execution stubbed at the seam (Option 1).** Full capability + status machine built/tested; the four external helpers (`_shopify_create_product`, `_printful_create_mockup`, `_printful_poll_mockup`, `_simulate_social_post`) return canned payloads; live Shopify/Printful wiring + the `mockup_url` artifact are a tracked demo-prep task. Printful polling is an **internal async loop**, not a second `LongRunningFunctionTool` (reconciles `02-architecture.md`:328 vs 402). **Channel selection by `product_route`** (D-030): poster/tshirt → shopify+printful; social_only/`None` → social. Execution failure is **retriable** (`execution=None` + approval stays `approved`), not a terminal failed state (MVP).
8. **(h) Layer-2 composition, not the anti-pattern.** The post-HITL branch is sanctioned layer-2 (`agentic-model.md`:56 — "when to redraft on `edit_requested`"), **not** a second strategic node, **not** an LLM re-planning the graph, **not** a post-approval mini-Workflow (revisit only when `record_outcomes` adds a second ordered post-approval step). The coordinator prompt's post-approval protocol is therefore **first-class** (the highest-risk surface; asserted by a behavioral repetition eval).
9. **(i) Display batch** — `request_human_approval(event_id)` reads `get_pending_approvals` (the authoritative `approval_id` carrier — **not** Step 6's `approval_id`-less `drafts`) and returns a batch **grouped by queue half** (exploitation / exploration via `discovery→exploration` mapping), each item carrying narrative + copy + `content_url`, all keyed by `approval_id`. Grouping is presentational; decision identity is `approval_id`. The **presentation surface** (a lightweight app posting the structured `FunctionResponse` — cleaner than parsing free-text chat) is a **separate demo-prep track**, not Step 7 agent scope.
10. **(j) Queue-level redo is designed-not-built.** Item-level copy `edit_requested` is in scope; rebuilding the *selection* (back to Step 5 with operator steering, a destructive rollback) is a future `repropose_queue` tool — **not** a per-item `ApprovalDecision` value, so deferring it carries **no retrofit cost** to the per-item contract.

**Eval shape:** coordinator-level, multi-turn, scripted operator `FunctionResponse` — a different seam from Steps 1–6's workflow-dispatch evals. A deterministic mocked **Tier-1 HITL+execution gate** is the merge gate (list round-trip, apply-before-branch, cap-survives-suspend, execution end-state); a live **behavioral protocol probe** (≥95%/20) measures the flash coordinator's protocol order — not a merge gate. New `approvals`/`campaigns` mock handlers follow the clobber rule (one combined handler per `(tool, collection)`, branching on `status`).

**Corrects:** `db-wrapper-inventory.md`:210, `02-architecture.md`:287-293 (HITL resume writes mis-attributed to the gate). **Refines:** D-024 (coordinator-over-workflow — capabilities now leave the graph for the coordinator plane), D-030 (closes the redraft/`operator_notes` reservation; reuses the route mapping), D-022 (decisions persisted on the approval doc; consumers re-read). **Closes:** `safety-measures.md` Gap 1.

---

### D-032 — `record_outcomes` Thin Honest Coda (Capability 9)

**Date:** 2026-05-29
**Decision:** Step 8 delivers capability **9 (`record_outcomes`)** — the 9th and **last** capability — as a **thin, honest coda**. It lives **coordinator-side** (registered as a `FunctionTool` in `build_coordinator()`), the **second coordinator-plane step** after D-031, and adds **zero** graph nodes (`build_pipeline_graph()` stays at 9). After `execute_approved_campaigns` publishes the approved set, `record_outcomes(event_id)` reads the published assets and writes **one provenance record per asset** to the `performance` collection, then the coordinator reports and stops. **Blessed at the human review gate 2026-05-29** (the descope touches Hard Constraint #1). Reference: `docs/plans/step-8-outcomes.md`, `docs/tasks/step-8-tasks.md`, `docs/_step-8-notes.md`.

**Sub-decisions:**
1. **(a) Descope-to-coda, not removal (Hard Constraint #1).** The coda writes a **provenance record only**: true linkage (`asset_id`/`campaign_id`/`event_id`), `channels` (awaiting measurement, derived from `product_route`), `window_start` (publish time), `window_days: 7`, `recorded_at`, and **`metrics: null`** with a **`metrics_status: "pending_sync"`** discriminator. **No fabricated metrics** — null means "not yet measured"; `{orders: 0}` would be a false claim nothing sold. The rich version (real engagement/conversion ingested **asynchronously** via Shopify order webhooks / channel analytics over the 7-day window, populating the nulls and flipping `metrics_status` to `synced`) is the named **enterprise path**. Capability 9 still runs end-to-end (read published → write provenance → report), so this is **descope + named external sync, not a dropped capability**. The coda's job is a *demonstrated* seam: it writes the exact row the sync would populate, and the demo points at where real outcomes plug in.
2. **(b) Spine protection — the discriminator must be honored by the reader, not just stamped by the writer.** Pending provenance docs land in the **same** `performance` collection that the Step-2 baseline reader (`aggregate_performance_for_events`, called by `build_event_context`) aggregates. A null-metric doc contributes 0 orders but +1 to `count`/`asset_count` — i.e. registers as a zero-performer and dilutes the baseline. Fix: prepend `{"$match": {"metrics_status": {"$ne": "pending_sync"}}}` to the baseline pipeline so it aggregates **measured-only** performance. This is a deliberate **retro into Step-2-owned code** — the `metrics_status` field doesn't exist until this step, so the reader can only be taught now. Filter is **`$ne "pending_sync"` (exclude-the-pending), not `$eq "synced"`** — so seeded/legacy docs with **no** status field are *kept* (backward-compatible; `$eq "synced"` would have silently dropped the seeded baseline). The discriminator is now a **contract on the `performance` collection**: any future reader (the deferred `get_top_performers_by_channel`, the enterprise sync's own queries) must also honor it. Proven by a **pending-neutral regression test** (baseline identical with/without pending docs). Protects the commercial spine the `_step-8-notes.md` insists on (correcting the loose earlier claim: performance is consumed via the **Step-2 historical baseline** off *measured* data, **not** via the Step-3 vector search, which never reads metrics).
3. **(c) Execute + record are sibling coordinator `FunctionTool`s, NOT a post-approval `Workflow`** (resolves the Step-7 flag, D-031(h)). `execute_approved_campaigns` already ships coordinator-side (D-031(a)); the deterministic `execute → record` tail sits downstream of the irreducibly coordinator-plane branch (redraft-loop vs. execute). Graph-wiring the tail would require pulling `execute` **back out of the coordinator plane into a node**, reopening D-031(a), for a trivial 2-node graph with no branch/reuse to gain. The **D-023** order-tension (order is prompt-enforced, not graph-enforced) is real and mitigated **defense-in-depth**: the coordinator prompt's post-approval protocol carries the order, **and** `record_outcomes` has a **tool-level no-op guard** — it returns `nothing_to_record` if no published assets exist, robust to LLM mis-ordering (keys on DB state, not prompt compliance). Revisited D-031(h), declined with reasons.
4. **(d) Idempotent upsert** by `(asset_id, event_id)` — execution is *retriable* (D-031(g)), so a re-run of execute→record must not double-write. **Deviation from the inventoried `insert-many`** (`db-wrapper-inventory.md`:235, updated this step).
5. **(e) No LLM call, no external API call** — `record_outcomes` is bounded computation end-to-end (read persisted state → write provenance → report). The **first purely-computational coordinator capability**; the cleanest terminal-text exit of the 9-capability arc (`agentic-model.md`:110). The coordinator's only involvement is *deciding to call it* after execute (a prompt-ordered dispatch).
6. **(f) `get_top_performers_by_channel` deferred** (designed-not-built) — the inventoried analytics aggregate, "optional for MVP." Demo-storytelling polish that the thin-coda direction deliberately doesn't make a claim on; bolts on later with zero retrofit cost. Blessed at the gate.

**Honest terminal text:** the coordinator reports "outcomes tracked, pending performance sync" — **never** claims sales/engagement numbers (none exist yet). Corrects the "metrics recorded" example in `agentic-model.md`:103.

**Corrects:** the "synthetic metrics" / "written simultaneously as synthetic values" framing in `01-requirements.md`:261, `_step-8-notes.md` § Honesty caveats item 1, and `agentic-model.md`:103 — all superseded by no-fabrication/provenance (the dated `_step-8-notes.md` § "Design fork — RESOLVED" already supersedes its own caveat). **Refines:** D-031 (second coordinator-plane capability; resolves D-031(h)), D-022 (consumers re-read persisted state), D-016/D-021 (the Step-2 baseline is the live performance-consumption path, now protected from pending writes). **Updates:** `db-wrapper-inventory.md`:235 (`record_performance` signature + upsert), `02-architecture.md`:143-172 + 311-315 (`performance` write shape + MCP block).
