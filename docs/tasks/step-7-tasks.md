...
- **Execution stubbed at the seam** — `_shopify_create_product` / `_printful_create_mockup` / `_printful_poll_mockup` / `_simulate_social_post` return canned payloads; Printful polling is an **internal async loop** (not a second `LongRunningFunctionTool`); CI never hits live APIs.
...
4. **External-helper stubs** — patch `_shopify_create_product` / `_printful_create_mockup` / `_printful_poll_mockup` / `_simulate_social_post` to canned payloads (Phase B).
...
Acceptance:
- Four **stubbed** internal helpers (the live/mock seam — bodies return canned payloads this step): `_shopify_create_product(...) -> dict`, `_printful_create_mockup(...) -> dict` (returns a `task_id`), `_printful_poll_mockup(task_id) -> dict` (returns `completed` + `mockup_url`), `_simulate_social_post(...) -> dict` (writes the social post package). Printful create+poll wrapped in an **internal async poll loop** (bounded retries + timeout) — **not** a second `LongRunningFunctionTool`; the stub returns `completed` immediately but the loop structure is real (demo-prep swaps only the helper body).
