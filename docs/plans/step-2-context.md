...
Step 2's surface exercises **failure categories 1, 3, 4, 5** per `docs/evaluation-strategy.md`: tool selection, tool arguments (correct `event_id`), tool-output handling (the hallucination guard is *category 4* — the model could correctly call the tool but the LLM-inside-tool could fabricate facts), end-state (narrative persisted with valid structure). Strategy coherence (category 6) does not apply until `propose_review_queue` ships.
